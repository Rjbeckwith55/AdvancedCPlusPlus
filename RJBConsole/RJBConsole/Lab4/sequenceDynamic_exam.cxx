...
// Revised by: CREngland (england@cod.edu) 09/19/2017
...
bool test_items(sequenceDynamic& test, size_t s, size_t i, double items[])
{
    ...
    cout << "The cursor should be at item [" << i << "]" << " of the sequenceDynamic\n";
    cout << "(counting the first item as [0]). I will advance the cursor\n";
    cout << "to the end of the sequenceDynamic, checking that each item is correct...";
    cout.flush( );
    ...
    {   // The test.is_item( ) function returns false too soon.
        cout << "\n    Cursor fell off the sequenceDynamic too soon." << endl;
        answer = false;
    }
    else if (i != s)
    ...
    {   // The test.is_item( ) function returns true after moving off the sequenceDynamic.
        cout << "\n    The cursor was moved off the sequenceDynamic,";
...
bool correct(sequenceDynamic& test, size_t size, size_t cursor_spot, double items[])
{
    ...
    {
        cout << "Test of the sequenceDynamic's items failed." << endl << endl;
        return false;
    }
    ...
    cout << "All tests passed for this sequenceDynamic." << endl << endl;
    return true;
}
...
int test2( )
{
    ...
    cout << "Using attach to put 20 and 30 in the sequenceDynamic, and then calling\n";
    ...
    cout << "Inserting 10, which should go at the sequenceDynamic's front." << endl;
    cout << "Then calling advance three times to run cursor off the sequenceDynamic ...";
    cout.flush( );
    test.insert(10);
    ...
    cout << test.DEFAULT_CAPACITY*10 << " at the sequenceDynamic's end." << endl;
    for (i = 4; i <= test.DEFAULT_CAPACITY; i++)
        test.attach(i*10);

    // Test that the sequenceDynamic is correctly filled.
    cout << "Now I will test that the sequenceDynamic has 10, 20, 30, ...";
    ...
    if (test.is_item( ))
    {
        cout << "    There are too many items on the sequenceDynamic." << endl;
        return false;
    }
...
int test3( )
{
    ...
    cout << "Using attach to build a sequenceDynamic with 10,30." << endl;
    test.attach(10);
    test.attach(30);
    cout << "Insert a 20 before the 30, so entire sequenceDynamic is 10,20,30." << endl;
    test.insert(20);
    if (!correct(test, 3, 1, items3)) return 0;
    cout << "Remove the 20, so entire sequenceDynamic is now 10,30." << endl;
    test.start( );
    test.advance( );
    test.remove_current( );
    if (!correct(test, 2, 1, items2)) return 0;
    cout << "Remove the 30, so entire sequenceDynamic is now just 10 with no cursor.";
    cout << endl;
    test.start( );
    test.advance( );
    test.remove_current( );
    ...
    cout << "Using attach to build another sequenceDynamic with 10,30." << endl;
    test.attach(10);
    test.attach(30);
    cout << "Insert a 20 before the 30, so entire sequenceDynamic is 10,20,30." << endl;
    test.insert(20);
    ...
    if (!correct(test, 2, 1, items2)) return 0;
    cout << "Set the cursor to the start and remove the 10," << endl;
    cout << "so the sequenceDynamic should now contain just 30." << endl;
    test.start( );
    test.remove_current( );
    if (!correct(test, 1, 0, items1)) return 0;
    cout << "Remove the 30 from the sequenceDynamic, resulting in an empty sequenceDynamic." << endl;
    test.start( );
    test.remove_current( );
    if (!correct(test, 0, 0, items1)) return 0;

    // Build a sequenceDynamic with three items 10, 20, 30, and remove the first.
    cout << "Build a new sequenceDynamic by inserting 30, 10, 20 (so the sequenceDynamic\n";
    ...
    cout << "Just for fun, I'll empty the sequenceDynamic then fill it up, then\n";
    cout << "empty it again. During this process, I'll try to determine\n";
    cout << "whether any of the sequenceDynamic's member functions access the\n";
    ...
    char_ptr = (char *) &test;
    for (i = 0; i < sizeof(sequenceDynamic); i++)
        if (char_ptr[i] == 'x')
        {
...
int test4( )
{
    ...
    cout << "I will now resize a sequenceDynamic to a larger capacity, and then\n";
    cout << "attach that many items. The sequenceDynamic should NOT need to\n";
    ...
    for (i = 0; i < 2*test.DEFAULT_CAPACITY; i++)
    {
        if (test.current( ) != i)
        {
            cout << "    sequenceDynamic does not contain correct items." << endl;
            return 0;
        }
        test.advance( );
    }
    ...
    if (mismatches > 1)
    {
        cout << "    sequenceDynamic was resized when it should not be." << endl;
        return 0;
    }
    else
        cout << "    Test passed." << endl;

    cout << "Now I will call resize(1) for the sequenceDynamic, but the actual\n";
    cout << "sequenceDynamic should not change because the sequenceDynamic already has \n";
...
int test5( )
{
    ...
    cout << "Copy constructor test: for an empty sequenceDynamic." << endl;
    ...
    cout << "Copy constructor test: for a sequenceDynamic with cursor at tail." << endl;
    ...
    cout << "Copy constructor test: for a sequenceDynamic with cursor near middle." << endl;
    original.insert(2);
    ...
    cout << "Copy constructor test: for a sequenceDynamic with no current item." << endl;
    original.insert(2);
...
int test6( )
{
    ...
    // Test copying of an empty sequenceDynamic. After the copying, we change the original.
    cout << "Assignment operator test: for an empty sequenceDynamic." << endl;
    ...
    // Test copying of a sequenceDynamic with current item at the tail.
    cout << "Assignment operator test: for a sequenceDynamic with cursor at tail." << endl;
    for (i=2; i <= 2*original.DEFAULT_CAPACITY; i++)
        original.attach(i);
    ...
    // Test copying of a sequenceDynamic with cursor near the middle.
    cout << "Assignment operator test: for a sequenceDynamic with cursor near middle." << endl;
    original.insert(2);
    for (i = 1; i < original.DEFAULT_CAPACITY; i++)
        original.advance( );
    // Cursor is now at location [DEFAULT_CAPACITY] (counting [0] as the first spot).
    ...
    // Test copying of a sequenceDynamic with no current item.
    cout << "Assignment operator test: for a sequenceDynamic with no current item." << endl;
    original.insert(2);
    while (original.is_item( ))
        original.advance( );
    // There is now no current item.
...
int main( )
{
    ...
    cout << "If you submit this sequenceDynamic class now, you will have\n";
